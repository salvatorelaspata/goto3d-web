...
export const RadioCard: React.FC<RadioCardInputProps> = ({
  ...
  return (
    <>
      ...
      <label
        htmlFor={`${id}--${value}`}
        className={`${selected === value && checked} shadow-md ${!disabled ? "hover:scale-105" : "bg-gray-200"} transition duration-300 ease-in-out justify-between w-full p-2 text-gray-500 bg-white border border-gray-200 rounded-lg ${!disabled && "cursor-pointer hover:bg-violet-200"} hover:text-gray-600`}
      >
