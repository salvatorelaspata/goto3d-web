...
export const RadioGroup: React.FC<RadioGroupProps> = ({id, label, name, icon, options, onChange, iValue, disabled = false}) => {
        ...
        <>
            ...
            <ul id={id} className={`grid w-full gap-4 ${gridCols} sm:grid-cols-1`}>
                ...
                <li key={value} className={`flex justify-between`}>
                    <RadioCard ref={ref} disabled={disabled} id={id} name={name} icon={icon} label={label} description={description} value={value} selected={selected} setSelected={setSelected} onChange={onChange}/>
                </li>
                ))}
            </ul>
